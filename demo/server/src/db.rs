...
impl Db {
    ...
    /// Runs inside a single WAL transaction for speed (~11 k rows in <300 ms).
            ...
            let tx = guard.transaction()?;
            {
                let mut upsert = tx.prepare_cached(
                    "INSERT INTO aircraft
                         (id,callsign,origin_country,altitude,velocity,heading,on_ground,updated_at)
                ...
                for a in &aircraft {
                    upsert.execute(params![
                        a.id, a.callsign, a.origin_country,
    ...
    /// Deletes position_history rows older than 10 minutes. Call every N polls.
    ...
    /// Fetches full metadata + last 3 position history from SQLite.
